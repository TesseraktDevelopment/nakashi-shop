...
export const Overview = () => {
	...
	const [rangedRevenue, setRangedRevenue] = useState<{
		value: number;
		percentage: number;
	}>({
		value: 0,
	...
	const [rangedOrders, setRangedOrders] = useState<{
		value: number;
		percentage: number;
	}>({
		value: 0,
	...
	useEffect(() => {
		const fetchCurrency = async () => {
			const { data } = await axios.get<ShopSetting>(
				"/api/globals/shopSettings",
	...
	const fetchRevenue = useCallback(
		async (
			...
			setRevenue: Dispatch<
				SetStateAction<{ value: number; percentage: number }>
			>,
			requestData?: { dateFrom?: string; dateTo?: string },
		) => {
			const { data } = await axios.post<RevenueResponse>(
				"/api/orders/revenue",
				requestData ?? {},
				{
					withCredentials: true,
	...
	useEffect(() => {
		void fetchOrderCount(rangedOrdersRef, setRangedOrders, {
			dateFrom,
			dateTo,
		});
		...
		<section className="flex flex-col gap-6">
			<div className="twp grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
				<OverviewCard
					...
					value={
						currency
							? formatPrice(totalRevenue.value, currency, locale)
							: totalRevenue.value
					}
				...
				<OverviewCard
					...
					value={
						currency
							? formatPrice(rangedRevenue.value, currency, locale)
							: rangedRevenue.value
					}
