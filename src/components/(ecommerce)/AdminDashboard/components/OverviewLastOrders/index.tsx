...
export const OverviewLastOrders = () => {
	...
	const columns: ColumnDef<Order>[] = [
		...
		},
		{
			...
			cell: ({ row }) => (
				<div className="lowercase">{row.original.shippingAddress.email}</div>
			),
		},
		{
			id: "amount",
			header: () => (
				<div className="text-right hover:bg-payload-elevation-0">
			...
			cell: ({ row }) => {
				...
				return (
					<DropdownMenu>
						...
						<DropdownMenuContent
							align="end"
							className="bg-payload-elevation-50"
						>
							<DropdownMenuLabel>
	...
	useEffect(() => {
		...
		const fetchOrders = async () => {
			try {
				const { data } = await axios.get<{ docs: Order[] }>(
					`/api/orders${stringifiedQuery}`,
					{
					...
					<div className="flex items-center py-4">
						...
						<DropdownMenu>
							<DropdownMenuTrigger asChild>
								...
								>
									{t("adminDashboard:columns")}{" "}
									<ChevronDown className="ml-2" width={20} height={20} />
								</Button>
							</DropdownMenuTrigger>
							<DropdownMenuContent
								align="end"
								...
								{table
									...
									.map((column) => {
										return (
											...
											>
												{t(
													`adminDashboard:${column.id as "date" | "status" | "email" | "amount"}`,
												)}
											</DropdownMenuCheckboxItem>
										);
									})}
									...
									>
										{headerGroup.headers.map((header) => {
											return (
												<TableHead
													key={header.id}
													className="text-payload-elevation-900 opacity-75"
												>
