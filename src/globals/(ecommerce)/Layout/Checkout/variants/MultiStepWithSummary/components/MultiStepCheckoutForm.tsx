...
import { type CheckoutFormData, useMultiCheckoutFormSchema } from "@/schemas/multiCheckoutForm.schema";
...
import { type PaymentMethod, type FilledCourier } from "../../OneStepWithSummary/components/CheckoutForm";
...
export const MultiStepCheckoutForm = ({
  ...
  const { CheckoutFormSchemaResolver, CartSchema, DeliverySchema, ShippingSchema } = useMultiCheckoutFormSchema();
  ...
  try {
    switch (currentStep) {
      case "cart": {
        ...
        return !!cart && cart.length > 0 && !!cartData.checkoutProducts && cartData.checkoutProducts.length > 0;
      }
      case "delivery": {
        const deliveryData = {
      ...
      case "summary": {
        await CheckoutFormSchemaResolver.parseAsync(form.getValues());
        return true;
      }
      default:
        return false;
    ...
    toast.error(t(`validation-error-${currentStep}`));
    return false;
  }
};
      ...
      <div className="lg:grid lg:grid-cols-2 lg:gap-x-12 xl:gap-x-16">
        ...
        <div className="mt-10 lg:sticky lg:top-28 lg:mt-0 lg:h-fit">
          ...
          <div className="mt-4 rounded-lg border border-gray-200 bg-white shadow-xs p-6">
            <p className="text-base font-medium text-gray-900">Nevíš si s něčím rady?</p>
            <p className="mt-1 text-sm text-gray-600">Rádi ti poradíme. Po–Pá 8:00–18:00.</p>
            <p className="mt-5 text-sm text-gray-600">Online chat</p>
            <p className="mt-1 text-sm text-gray-600">podpora@nakashi-army.cz</p>
          </div>
        </div>
      </div>
    </>
