//MultiStepWithSummary/components/DeliveryMethod
"use client";
...
export const DeliveryMethod = ({
	...
	zasilkovnaSettings?: {
		apiKey: string;
		deliveryZones: [countries: string[]];
		countries: string[];
		language: string;
		filterTypes?: string[];
		weight?: string;
	...
	const pickupPointID = useWatch({
		control: form.control,
		name: "shipping.pickupPointID",
	});
	const pickupPointName = useWatch({
		control: form.control,
		name: "shipping.pickupPointName",
	});
	const pickupPointAddress = useWatch({
		control: form.control,
		name: "shipping.pickupPointAddress",
	});
				...
				<div className="mt-2 flex flex-row-reverse">
					<ZasilkovnaWidget
						...
						options={{
							...
							valueFormat:
								'"Packeta",id,carrierId,carrierPickupPointId,name,city,street,branchCode',
							view: "modal",
							vendors: [
								{
									country: zasilkovnaSettings?.countries[0] ?? "cz",
									group: "zbox",
								},
								{
									country: zasilkovnaSettings?.countries[1] ?? "sk",
									group: "zbox",
								},
							],
		...
		<div className="flex flex-1 flex-col">
			<span className="flex flex-1 items-center gap-3">
				{icon?.url && (
					<Media
						resource={icon}
						className="block aspect-square max-h-12 w-fit max-w-[62px]"
					/>
				)}
