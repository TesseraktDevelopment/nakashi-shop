...
export const CheckoutForm = ({
	...
	const fetchAresOrsrData = useCallback(
		async (ico: string, country: "cz" | "sk") => {
			...
			try {
				if (country === "cz") {
					const response = await axios.get<AresResponse>(
						`/next/services/ares?ico=${ico}`,
						{
					...
					if (data?.obchodniJmeno) {
						...
						form.setValue(
							"invoice.postalCode",
							data.sidlo?.psc ? String(data.sidlo.psc) : "",
						);
							...
							<div className="mt-4 grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-4">
								{shippingAddresses ? (
									<div
										...
										onKeyDown={(e) =>
											e.key === "Enter" && setShippingDialogOpen(true)
										}
									>
								...
								{(wantsInvoice || isCompany) && (
									<>
										...
										<FormField
											...
											render={({ field }) => (
												<FormItem>
													<FormLabel htmlFor="address-level2">
														{t("city")}
													</FormLabel>
													<FormControl>
														<Input
									...
									render={({ field }) => (
										<FormItem>
											<RadioGroup
												value={field.value}
												onChange={(value) => {
													const recommendedMethod = paymentMethods.find(
														(m) => m.recommended && !m.disabled,
													)?.id;
