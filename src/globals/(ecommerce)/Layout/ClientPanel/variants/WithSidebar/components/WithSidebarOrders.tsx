					...
					<Card className="flex flex-col overflow-clip rounded-[6px] border bg-transparent sm:flex-row">
						<div className="r flex flex-col gap-2 bg-gray-50 p-4 sm:w-1/3 sm:pr-8">
							{t.rich(order.orderDetails.status, {
								yellow: (chunks) => (
									<p className="font-medium text-yellow-600">{chunks}</p>
								),
								green: (chunks) => (
									<p className="font-medium text-green-600">{chunks}</p>
								),
								red: (chunks) => (
									<p className="font-medium text-red-600">{chunks}</p>
								),
							})}
							...
							<p className="font-medium">
								{formatPrice(
									order.orderDetails?.total ?? 0,
									order.orderDetails?.currency ?? "CZK",
									locale,
								)}
							</p>
						</div>
						<div className="flex items-center gap-2 overflow-y-auto p-4 sm:pl-8">
							{order.products?.map((product) => {
								...
								const image =
									variantImage && typeof variantImage !== "string"
										? variantImage
										: productImage;
