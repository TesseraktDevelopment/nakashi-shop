...
export const createPacketaZBoxShipment = async (
	...
	try {
		parsed = (await parser.parseStringPromise(
			rawResponse,
	...
	const checkShipmentStatus = async (maxAttempts = 10): Promise<string> => {
		for (let attempt = 0; attempt < maxAttempts; attempt++) {
			try {
				...
				console.warn(
					`Attempt ${attempt + 1} failed while checking shipment status.`,
				);
			}
		...
		console.warn(
			"Shipment status not confirmed as accepted, but barcode already received",
		);
