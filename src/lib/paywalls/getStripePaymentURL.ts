...
export const getStripePaymentURL = async ({
	...
	const stripeMappedProducts: Stripe.Checkout.SessionCreateParams.LineItem[] =
		filledProducts.map((product) => {
			...
			if (product.enableVariantPrices && product.variant?.pricing) {
				productPrice = product.variant.pricing.find(
					(price) => price.currency === currency,
				)?.value;
			} else if (product.pricing) {
				productPrice = product.pricing.find(
					(price) => price.currency === currency,
				)?.value;
			}
			...
			if (!productPrice) {
				...
				throw new Error(
					`Product price not found for product: ${product.id}, title: ${product.title}`,
				);
			}
	...
	try {
		console.log(
			"Stripe line_items:",
			JSON.stringify(stripeMappedProducts, null, 2),
		);
		...
		const sessionConfig: Stripe.Checkout.SessionCreateParams = {
			...
			shipping_options: [
				{
					shipping_rate_data: {
						...
						display_name:
							`${shippingLabel}${pickupPointID ? ` (${pickupPointID})` : ""}` ||
							"Doprava (Shipping)",
						delivery_estimate: {
			...
			customer_email: customerId
				? undefined
				: (checkoutData.shipping.email ?? "unknown@example.com"),
			tax_id_collection: {
				enabled: true,
			},
		};
