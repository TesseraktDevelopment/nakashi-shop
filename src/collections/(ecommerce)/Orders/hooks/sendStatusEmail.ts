...
export const sendStatusEmail: FieldHook<
	Order,
	Order["orderDetails"]["status"] | undefined
> = async ({ operation, value, originalDoc }) => {
	if (operation !== "update" || !originalDoc || !value) return value;

	const disabledStatuses: Order["orderDetails"]["status"][] = [
		"cancelled",
		"completed",
		"pending",
	];
