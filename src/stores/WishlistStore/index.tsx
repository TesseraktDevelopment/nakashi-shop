...
const fetchWishListFromUserAccount = async (): Promise<WishList | null> => {
	try {
		const { data } = await axios.get<{ data: WishList; status: number }>(
			"/next/wishlist",
		);
...
const debouncedFetchWishListFromUserAccount = debounce(
	fetchWishListFromUserAccount,
	1000,
);

const debouncedSaveWishListToUserAccount = debounce(
	saveWishListToUserAccount,
	1000,
);
...
const useWishListStore = create<WishListState>((set) => ({
		...
		? (() => {
				...
				if (wishlistData && wishlistData.length > 1) {
					try {
						...
						console.error(
							"Error parsing wishlist data from localStorage",
							error,
						);
						return [];
					}
	...
	synchronizeWishList: async () => {
		...
		const wishlistFromUserAccount =
			await debouncedFetchWishListFromUserAccount();
		...
		window.localStorage.setItem(
			"wishlist",
			JSON.stringify(wishlistFromUserAccount),
		);
