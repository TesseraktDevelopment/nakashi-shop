...
export const getRevenue = async (req: PayloadRequest) => {
	try {
		...
		const dates =
			req.json &&
			((await req.json()) as { dateFrom?: string; dateTo?: string });

		const dateFrom =
			dates?.dateFrom && new Date(dates.dateFrom).setHours(0, 0, 0, 0);
		const dateTo =
			dates?.dateTo && new Date(dates.dateTo).setHours(23, 59, 59, 999);
		...
		const { availableCurrencies, currencyValues } = await getCachedGlobal(
			"shopSettings",
			"en",
		)();
				...
				.reduce((acc: number, doc: Order) => {
					...
					return (
						...
						doc.orderDetails.total /
							(currencyValues?.find(
								(currency) => currency?.currency === doc.orderDetails.currency,
							)?.value ?? 1)
					);
				}, 0)
				.toFixed(2),
