	...
	try {
		...
		switch (event.type) {
			case "checkout.session.completed": {
				...
				if (!orderID) {
					console.error(
						"No orderID found in checkout.session.completed metadata",
					);
					return Response.json({
						status: 400,
				...
				if (!paymentIntentId) {
					console.error(
						"No payment_intent ID found in checkout.session.completed",
					);
					return Response.json({
						status: 400,
						message: "Missing payment_intent ID",
					});
				}

				console.log(
					`Processing checkout.session.completed for orderID: ${orderID}`,
				);
				...
				try {
					...
					console.log(
						`Order ${orderID} updated to paid with transactionID: ${paymentIntentId}`,
					);
				} catch (updateError) {
			...
			case "payment_intent.succeeded": {
				...
				if (!orderID) {
					console.error(
						"No orderID found in payment_intent.succeeded metadata",
					);
					return Response.json({
						status: 400,
						message: "Missing orderID in metadata",
				...
				if (paymentIntent.status === "succeeded") {
					try {
						...
						console.log(
							`Order ${orderID} updated to paid with transactionID: ${paymentIntent.id}`,
						);
					} catch (updateError) {
						console.error(`Failed to update order ${orderID}:`, updateError);
						return Response.json({
			...
			case "payment_intent.canceled": {
				...
				try {
					...
					console.log(
						`Order ${orderID} updated to ${event.type === "payment_intent.canceled" ? "cancelled" : "unpaid"} with transactionID: ${paymentIntent.id}`,
					);
				} catch (updateError) {
					console.error(`Failed to update order ${orderID}:`, updateError);
					return Response.json({
			...
			case "customer.deleted": {
				...
				console.log(
					`Processing customer.deleted for customerId: ${customerId}`,
				);
				...
				try {
					...
					console.log(
						`Customer with Stripe ID ${customerId} updated to remove Stripe ID`,
					);
				} catch (updateError) {
					console.error(
						`Failed to update customer with Stripe ID ${customerId}:`,
						updateError,
					);
					return Response.json({
						status: 500,
						message: `Failed to update customer with Stripe ID ${customerId}`,
					});
				}
				break;
			}
