...
export async function POST(req: Request) {
	const { getCouriersArray } = await import(
	...
	try {
		...
		const {
			...
			selectedCountry,
			locale,
		}: { cart: Cart | undefined; selectedCountry: Country; locale: Locale } =
			(await req.json()) as {
				cart: Cart | undefined;
		...
		const filledCouriers = couriers
			.filter((courier) =>
				courier?.deliveryZones?.find((zone) =>
					zone.countries.includes(selectedCountry),
				),
			)
			.map((courier) => {
				const deliveryZone = courier?.deliveryZones?.find((zone) =>
					zone.countries.includes(selectedCountry),
				);
					...
					(prices) => {
						...
						if (!freeShippingValue || !totalPriceInCurrency) {
							...
							return {
								...prices,
								value:
									totalPriceInCurrency >= freeShippingValue ? 0 : prices.value,
							};
						}
					},
				);
		...
		const productsWithTotalAndCouriers = {
			...
			totalQuantity: filledProducts.reduce(
