...
export async function GET(req: Request) {
  try {
    ...
    if (!orderId || !locale) {
      ...
      return Response.json({ status: 400, message: "Missing orderId or locale" });
    }
    ...
    if (order.orderDetails.status !== "unpaid" && order.orderDetails.status !== "cancelled") {
      console.log(`Order ${orderId} is not in unpaid or cancelled state, status: ${order.orderDetails.status}`);
      return Response.json({ status: 400, message: "Order is not eligible for retry payment" });
    }
    ...
    const { stripe: stripeOptions } = await getCachedGlobal("paywalls", locale, 1)();
    const stripe = new Stripe(stripeOptions?.secret ?? "", { apiVersion: "2025-07-30.basil" });
    ...
    if (order.orderDetails.stripeSessionId) {
      try {
        ...
        const session = await stripe.checkout.sessions.retrieve(order.orderDetails.stripeSessionId);
        if (session.status === "open" && session.url) {
          stripePaymentURL = session.url;
          console.log(`Using existing Stripe session ${order.orderDetails.stripeSessionId} for order ${orderId}`);
        } else {
          console.log(`Stripe session ${order.orderDetails.stripeSessionId} is expired or invalid`);
        }
      } catch (error) {
        console.error(`Error retrieving Stripe session ${order.orderDetails.stripeSessionId}:`, error);
      }
    }
    ...
    if (!stripePaymentURL) {
      ...
      const checkoutData: CheckoutFormData = {
        ...
        shipping: {
          ...
          pickupPointAddress: order.shippingAddress.pickupPointAddress ?? undefined,
          ...
          pickupPointBranchCode: order.shippingAddress.pickupPointBranchCode ?? undefined,
        },
