...
const OrdersPage = async ({
	...
	searchParams: Promise<{ x?: string; cancelled?: string }>;
}) => {
			...
			<div className="bg-white">
				<div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 sm:py-24 lg:px-8">
					<div className="max-w-xl">
						...
						<p className="mt-2 text-base text-gray-500">
							The order with ID #{id} could not be found. Please check the order
					...
					<div className="max-w-2xl">
						...
						<div className="mt-10 border-t border-gray-200">
							...
							<div className="space-y-6">
								{Array.from({ length: order.products?.length ?? 3 }).map(
									(_, index) => (
										<div
											key={index}
		...
		try {
			...
			if (result.success) {
				...
				if (data.status === 200 && data.url) {
					...
					console.error(
						"Failed to fetch retry payment URL:",
						data.message ?? "Unknown error",
					);
				}
			} else {
				console.error(
					"Invalid retry payment response:",
						...
						<dl className="grid grid-cols-2 gap-x-6 border-gray-200 py-10 text-sm">
							<div>
								...
								<dd className="mt-2 text-gray-700">
									<address className="not-italic">
										<span className="block">
											{order.orderDetails.status === "paid"
												? "Stripe"
												: "Čeká na platbu"}
										</span>
